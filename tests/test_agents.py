...
"""
    ...
    assert agent.config.app_data_dir is not None
...
def test_agent_inputs_mapping(tmp_path):
    ...
    # TextPart
    inputs: list[AgentInputPart] = [TextPart(text="hello")]
    ...
    inputs: list[AgentInputPart] = [ImagePart.from_file(str(img_path))]
    prepared = agent._prepare_inputs(inputs)
    ...
    inputs: list[AgentInputPart] = [ImagePart.from_bytes(b"data bytes", mime_type="image/jpeg")]
    prepared = agent._prepare_inputs(inputs)
    assert len(prepared) == 1
