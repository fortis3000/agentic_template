...
""",
    ...
    with patch("src.agents.google_antigravity.G_Agent") as mock_g_agent, \
        ...
        assert agent_span.attributes.get(SpanAttributes.OPENINFERENCE_SPAN_KIND) == OpenInferenceSpanKindValues.AGENT.value
        ...
        assert tool_span.attributes.get(SpanAttributes.OPENINFERENCE_SPAN_KIND) == OpenInferenceSpanKindValues.TOOL.value
        assert tool_span.attributes.get(SpanAttributes.TOOL_NAME) == "custom_tool"
        assert "My test tool description" in tool_span.attributes.get(SpanAttributes.TOOL_DESCRIPTION)
