...
| **`Mutmut Mutation Testing`** | `src/utils/*.py`, `src/tools/local/text_extractor.py`, `src/agents/config.py` | Injects synthetic mutations to assess test suite effectiveness and catch untested logic branches. | `make mutate` |
...
Mutation testing with `mutmut` evaluates test quality by introducing synthetic bugs (mutants) into source files (`src/utils/retry.py`, `src/utils/pr_validator.py`, `src/tools/local/text_extractor.py`, `src/agents/config.py`) and running target pytest suites (`tests/test_retry.py`, `tests/test_pr_validator.py`, `tests/test_text_extractor.py`, `tests/test_phoenix_config.py`).
